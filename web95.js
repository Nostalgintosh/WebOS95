...
const AS_EXT = /^(chrome|moz)-extension:$/.test(location.protocol);
...
const LOGO = "Windows_Logo_(1992-2001).svg";
const WALLPAPER_PRESETS = [
  { name:"Classic Flag", src:LOGO, mode:"fit", size:45, color:"#008080" },
...
/* Authentic Windows 95 shell icons, lifted from the icon pack in assets/.
   The three terminal glyphs stay hand-drawn: they are tinted per shell
   (PowerShell blue, bash black, zsh charcoal), which a single MS-DOS icon
   cannot convey. Everything else is the real artwork. */
const ICONS = {
  ...
  /* Start button: the small pixel flag. The big wavy logo is the wallpaper. */
  win: "assets/flag.ico",
...
const defaults = () => ({
  ...
  notes: { welcome: "MiniOS 95 scratch pad\n================\n\nThis text is saved in your browser.\n" },
  iconPos: {},
  fs: defaultFS()
});
...
function load(){
  try{
    ...
    const d = defaults();
    return Object.assign(d, s, { fs: s.fs || d.fs, notes: s.notes || d.notes, iconPos: s.iconPos || {} });
  }catch(e){ return defaults(); }
}
function wipe(){
  try{ localStorage.removeItem(KEY); localStorage.removeItem(KEY + ".seen"); }catch(e){}
}
let saveTimer=null;
function save(){
...
function blockedAddress(url, why){
  ...
  dialog({
    ...
    message: (internal
      ...
      : "This page cannot open local files unless you allow it.\n\nOn the extensions page, open MiniOS 95's details and turn on “Allow access to file URLs” — or paste this into the address bar:")
      + (why ? "\n\n(" + why + ")" : ""),
...
function makeWindow(opts){
  ...
  tbar.addEventListener("dblclick", e => { if(e.target.tagName !== "BUTTON") w.toggleMax(); });
...
function dragify(handle, node, w){
  handle.addEventListener("pointerdown", e => {
    if(e.target.tagName === "BUTTON" || w.max) return;
    handle.setPointerCapture(e.pointerId);
    const sx = e.clientX, sy = e.clientY;
    const ox = node.offsetLeft, oy = node.offsetTop;
    const move = ev => {
      const nx = Math.min(Math.max(ox + ev.clientX - sx, -node.offsetWidth + 60), desktop.clientWidth - 40);
      const ny = Math.min(Math.max(oy + ev.clientY - sy, 0), desktop.clientHeight - 24);
...
function resizify(grip, node, w){
  grip.addEventListener("pointerdown", e => {
    ...
    const move = ev => {
      node.style.width  = Math.max(240, ow + ev.clientX - sx) + "px";
      node.style.height = Math.max(140, oh + ev.clientY - sy) + "px";
      if(w.onResize) w.onResize();
    };
...
function dialog(opts){
  ...
  return new Promise(resolve => {
    ...
    const tt = el("div","tt", opts.title || "MiniOS 95");
...
const say = (message, title) => dialog({ title: title || "MiniOS 95", message, msgIcon: ICONS.help, buttons:[{label:"OK",value:true,primary:true}] });
const ask = (message, title) => dialog({ title: title || "Confirm", message, msgIcon: ICONS.warn,
...
document.addEventListener("pointerdown", e => {
  if(!ctx.contains(e.target) && !$("#startmenu").contains(e.target) && !$("#start").contains(e.target)) hideMenus();
});
...
/* ============================================================
   ...
   ============================================================ */
const SYSTEM_ICONS = [
  { id:"sys:computer", name:"My Computer", icon:ICONS.computer, action:()=>openExplorer(state.fs) },
  ...
  { id:"sys:settings", name:"Settings",    icon:ICONS.settings, action:()=>openSettings() },
...
function renderIcons(){
  ...
  const entries = [];
  SYSTEM_ICONS.forEach(s => entries.push({ key:s.id, name:s.name, icon:(typeof s.icon==="function"?s.icon():s.icon), sys:s }));
  ...
  entries.forEach(e => {
    ...
    d.addEventListener("pointerdown", ev => {
      ...
      const mv = m => {
        ...
        moved = true;
        d.style.left = Math.max(0, Math.min(ox + m.clientX - sx, desktop.clientWidth - 78)) + "px";
        d.style.top  = Math.max(0, Math.min(oy + m.clientY - sy, desktop.clientHeight - 70)) + "px";
      };
      const up = () => {
        d.removeEventListener("pointermove", mv); d.removeEventListener("pointerup", up);
        if(moved){ state.iconPos[e.key] = { x:d.offsetLeft, y:d.offsetTop }; save(); }
      };
...
desktop.addEventListener("contextmenu", e => {
  if(e.target.closest(".win") || e.target.closest("#omni") || e.target.closest("#today")) return;
  e.preventDefault();
...
function openExplorer(folder){
  ...
  const winTitle = f => f === state.fs ? "My Computer (C:)" : f.name;
  ...
  ["File","Edit","View","Help"].forEach(m => {
    ...
    s.onclick = ev => {
      ...
      if(m === "File") menu(r.left, r.bottom, [...newItemsMenu(st.folder), "-", { label:"Close", action:()=>w.close() }]);
        ...
        { label:"Sort by name", action:()=>{ st.folder.children.sort((a,b)=> (isFolder(b)-isFolder(a)) || a.name.localeCompare(b.name)); save(); refreshAll(); } },
      ...
      else menu(r.left, r.bottom, [{ label:"About MiniOS 95", action:()=>openHelp() }]);
    };
    mb.appendChild(s);
  });
  ...
  function render(){
    ...
    const f = st.folder;
    addrText.textContent = pathString(f, "win");
    pane.innerHTML = "";
    const kids = (f.children||[]).slice().sort((a,b)=> (isFolder(b)-isFolder(a)) || a.name.localeCompare(b.name));
    ...
    s2.textContent = f === state.fs ? "My Computer" : "";
...
function openTerminal(shell){
  ...
  function text(s, cls){ return out(esc(s == null ? "" : s), cls); }
...
function banner(S){
  ...
  if(S.shell === "powershell"){
    S.text("Windows PowerShell (MiniOS 95 Edition)");
...
function resolve(spec, cwd){
  spec = String(spec).trim().replace(/^["']|["']$/g, "");
  if(!spec) return cwd;
  let node = cwd;
  let s = spec;
  if(/^([a-z]:)?[\\/]/i.test(s) || s === "~" || s.startsWith("~/") || s.startsWith("~\\")){
    node = state.fs;
...
const promptHTMLPlain = S => "";

/* --- command implementations ------------------------------------ */
function fmtList(S, folder){
  const kids = (folder.children||[]).slice().sort((a,b)=> (isFolder(b)-isFolder(a)) || a.name.localeCompare(b.name));
...
function needArg(S, cmd){ S.text(cmd + ": missing operand", "err"); }
...
cmd("help man get-help ?", "Show this list", (S) => {
  S.text("");
  S.out('<span class="hd">MiniOS 95 shell — ' + SHELLS[S.shell].label + '</span>');
  S.text("");
  const seen = new Set();
...
cmd("tree", "Show the whole bookmark tree", (S, args) => {
  ...
  (function walk(n, pre){
    const kids = (n.children||[]).slice().sort((a,b)=> (isFolder(b)-isFolder(a)) || a.name.localeCompare(b.name));
    kids.forEach((c,i) => {
...
cmd("neofetch winfetch about ver", "System info", (S) => {
  ...
  const info = [
    ...
    "-----------------",
    "OS: MiniOS 95 " + (AS_EXT ? "(new tab edition)" : "(file edition)"),
...
cmd("reset factory-reset", "Wipe everything and start fresh", async (S) => {
  if(await ask("This erases all your folders, bookmarks and notes. Continue?", "Reset MiniOS 95")){
    wipe();
    state = defaults(); reindex(); save(); applyChrome(); refreshAll();
...
function openNotepad(key, seed, title){
  ...
  ["File","Edit","Help"].forEach(m => {
    ...
    s.onclick = () => {
      ...
      else menu(r.left, r.bottom, [{ label:"About Notepad", action:()=>say("MiniOS 95 Notepad.\nText is stored in this browser only.","About") }]);
    };
    mb.appendChild(s);
  });
...
function openMiniEditor(){
  ...
  ["File","Edit","View","Language","Help"].forEach(name => {
    ...
    item.onclick = () => {
      ...
      else menu(r.left,r.bottom,[{label:"Keyboard shortcuts",action:()=>say("Ctrl+N  New document\nCtrl+O  Open text file\nCtrl+S  Save locally\nCtrl+F  Find\nF3  Find next\nCtrl+H  Replace all\nCtrl+G  Go to line\nCtrl++ / Ctrl+-  Zoom\nTab / Shift+Tab  Indent / outdent","MiniEditor shortcuts")},"-",{label:"About MiniEditor",action:()=>say("A small, focused editor for MiniOS 95. Documents autosave in this browser, and can be opened from or downloaded to your computer.","MiniEditor")}]);
    };
    mb.appendChild(item);
  });
  ...
  editor.addEventListener("dragleave",e=>{ if(!editor.contains(e.relatedTarget)) editor.classList.remove("drop-target"); });
...
function openSettings(){
  ...
  const w = makeWindow({ title:"Display & Shell Properties", icon:ICONS.settings, kind:"settings", w:520, h:470 });
  ...
  Object.keys(SHELLS).forEach(k => {
    ...
    r.onchange = () => { state.shell = k; save(); applyChrome(); };
  ...
  [["same","This tab"],["new","New tab"]].forEach(([v,lbl]) => {
    ...
    r.onchange = () => { state.linkTarget = v; save(); };
  ...
  ["#008080","#3a6ea5","#000000","#5c5c8a","#7e9c7e","#a08050"].forEach(c => {
  ...
  [["fit","Fit"],["center","Centre"],["tile","Tile"],["stretch","Fill screen"]].forEach(([v,lbl]) => {
    ...
    r.onchange = () => { state.wallpaperMode = v; save(); applyWallpaper(); };
  ...
  bReset.onclick = async () => {
    if(await ask("Erase all folders, bookmarks, notes and settings?","Reset MiniOS 95")){
      wipe(); state = defaults(); reindex(); save(); applyChrome(); refreshAll();
    }
  };
...
async function importFromPaste(into){
  const r = await dialog({ title:"Import bookmarks.html",
    message: (AS_EXT
      ? "MiniOS 95 can read your bookmarks directly if you grant it permission — click “Import browser bookmarks…” again and accept the browser's prompt.\n\nOr do it by hand: bookmark manager (Ctrl+Shift+O) → ⋮ → Export bookmarks, open the saved file in a text editor, and paste it here."
...
function openHelp(){
  ...
  if(ex){ focusWin(ex); return; }
  const w = makeWindow({ title:"Read Me — MiniOS 95", icon:ICONS.help, kind:"help", w:600, h:440 });
  const box = el("div","fields in"); box.style.background = "#fff"; box.style.userSelect = "text";
  box.innerHTML = `
    <h2 style="margin:0 0 6px;font-size:15px">MiniOS 95</h2>
    <div class="hint" style="margin-bottom:10px">A new tab page that behaves like a small desktop. Folders hold your links; a terminal drives the same tree from PowerShell, Bash or Zsh.</div>
    ...
    <fieldset><legend>Shortcuts</legend>
      <div class="hint">
        ...
        <kbd>Ctrl</kbd>+<kbd>\`</kbd> new terminal · <kbd>Ctrl</kbd>+<kbd>K</kbd> focus search · <kbd>Ctrl</kbd>+<kbd>E</kbd> My Computer ·
  ...
  if(AS_EXT){
    where.innerHTML =
      `Loaded as the <b>MiniOS 95</b> extension, so it already replaces <b>every new tab</b>.<br><br>` +
...
function buildStart(){
  ...
  list.appendChild(mkItem("Programs", ICONS.computer, null, s => {
    [["Terminal — PowerShell", ICONS.terminal, ()=>openTerminal("powershell")],
     ...
     ["My Computer", ICONS.computer, ()=>openExplorer(state.fs)]
    ].forEach(([l,ic,a]) => s.appendChild(mkItem(l, ic, a)));
  }));
  list.appendChild(mkItem("Favorites", ICONS.folder, null, s => {
  ...
  list.appendChild(mkItem("Settings", ICONS.settings, ()=>openSettings()));
  list.appendChild(mkItem("Find…", ICONS.find, ()=>{ $("#q").focus(); $("#q").select(); }));
  ...
  list.appendChild(mkItem("Run…", ICONS.run, async ()=>{
    const r = await dialog({ title:"Run", msgIcon:ICONS.run,
      message:"Type a command, a bookmark name or an address, and MiniOS 95 will open it.",
  ...
  list.appendChild(mkItem("Shut Down…", ICONS.shutdown, async ()=>{
    if(await ask("Are you sure you want to shut down MiniOS 95?","Shut Down Windows")) $("#shutdown").classList.add("open");
  }));
}
$("#start").onclick = e => {
  e.stopPropagation();
  ...
  if(open){ buildStart(); m.style.zIndex = ++zTop; }
};
...
$("#omniform").addEventListener("submit", e => {
  ...
  if(looksLikeUrl(v)) return go(v);
  const n = resolve(v, state.fs);
  if(n){ openNode(n); $("#q").value = ""; return; }
  go(searchUrl(v));
});
...
function applyChrome(){
  ...
  const si = $("#start").querySelector("img");
  si.src = ICONS.win;
...
document.addEventListener("keydown", e => {
  ...
  if(e.ctrlKey && (e.key === "k" || e.key === "K")){ e.preventDefault(); $("#q").focus(); $("#q").select(); return; }
